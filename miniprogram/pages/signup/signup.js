...
Page({
  data: { publicId: '', data: null, loading: true, modal: false, selectedSlot: null, personType: 'member', members: [], memberLabels: [], memberIndex: 0, name: '', club: '', submitting: false },
  onLoad(options) { this.setData({ publicId: options.publicId || '' }); this.load(); this.loadMembers(); },
  onShow() { if (this.data.publicId) this.load(); },
  ...
  openSignup(event) { const slotId = event.currentTarget.dataset.slotId || ''; const slot = slotId && this.data.data.slots.find((item) => item.id === slotId); this.setData({ modal: true, selectedSlot: slot || null, personType: 'member', name: '', club: '' }); },
  ...
  async submitSignup() {
    ...
    try { const data = await cloud.callCloud('signupService', payload); this.setData({ data, modal: false }); cloud.showSuccess('报名成功'); }
    catch (error) { cloud.showError(error); await this.load(); }
