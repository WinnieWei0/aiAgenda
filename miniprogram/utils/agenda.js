...
function validateAgendaForPreview(agendaValue) {
  ...
  const errors = [];
  const hiddenSectionIds = new Set(['evaluation', 'facilitatorReport', 'vote']);
  const hiddenRowIds = new Set(['guestIntroduction', 'topicNote', 'topicSummary', 'openingIcebreaker', 'feedback', 'award']);
  ...
  const validateRow = (row, label, isSectionRow) => {
    const legacyIcebreaker = row && row.moduleKind === 'icebreaker' && row.dynamic !== true;
    if (!row || hiddenRowIds.has(row.id) || legacyIcebreaker) {
      return;
    }
    if (row.moduleKind === 'workshop') {
      addTextError(`${label}主题`, row.topic);
    }
